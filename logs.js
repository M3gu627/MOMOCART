...
document.addEventListener('click', function(e) {
    ...
    if (invoiceBtn) {
        ...
        if (id) { window._currentInvoiceLogId = id; openInvoiceModal(id); }
        return;
    }
});
...
async function loadLogs() {
    // Pass selected date so API only returns that day — stays fast as DB grows.
    const dateEl = document.getElementById('header-date');
    const date   = dateEl ? dateEl.value : '';
    const dateParam = date ? `&date=${date}` : '';
    try {
        const [logsRes, expRes, depRes] = await Promise.all([
            apiFetch(`api.php?action=getLogs${dateParam}`),
...
// Full fetch (no date filter) — used by Weekly/Monthly/Yearly/Daily summary tabs and Export.
async function loadAllLogs() {
    try {
        const [logsRes, expRes, depRes] = await Promise.all([
            apiFetch('api.php?action=getLogs&date=all'),
            apiFetch('api.php?action=getExpenses'),
            apiFetch('api.php?action=getDeposits')
        ]);
        const logsData = await logsRes.json();
...
function renderApp() {
    ...
    if (currentUser.role === 'admin') {
        ...
        const thEmpName = document.getElementById('th-employee-name');
        if (thEmpName) thEmpName.classList.remove('hidden');
        ...
        // Hide date picker for branch accounts — admin only
        const dateWrap = document.getElementById('header-date-wrap');
        ...
        const branchInput = document.getElementById('header-branch');
        if (branchInput) branchInput.value = (currentUser.displayName || currentUser.username || 'Branch').toUpperCase() + ' LOGBOOK';

        // Show Employee Name column for branch accounts too
        const thEmpNameBranch = document.getElementById('th-employee-name');
        if (thEmpNameBranch) thEmpNameBranch.classList.remove('hidden');
...
function renderTable() {
    ...
    const dateEl = document.getElementById('header-date');
    let selectedDate = dateEl ? dateEl.value : '';

    // Fallback: if date is somehow empty, default to today in Philippine local time
    if (!selectedDate) {
        const now = new Date();
    ...
    paginatedLogs.forEach((log, index) => {
        ...
        // No. column
        html += `<td class="px-4 py-4 text-slate-400">${startIndex + index + 1}</td>`;

        // Employee Name column — shown for all roles
        html += `<td class="px-4 py-4 text-slate-600">${log.employee_name || '<span class="text-slate-300">—</span>'}</td>`;
...
// exportToCSV() has been moved to export.js — it auto-dispatches per active tab.
...
function switchTab(tab) {
    ...
    if (tab === 0) {
        ...
        loadLogs(); // re-fetch date-filtered records
    } else if (tab === 1) {
        document.getElementById('daily-content').classList.remove('hidden');
        loadAllLogs(); // needs full history
    } else if (tab === 2) {
