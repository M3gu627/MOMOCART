...
function handleValidIdChange() {
    ...
    const allOptions = [
        ...
        { value: '15kiddie',  text: '15 Mins (Kiddie Cart)',  forKiddie: true,  forRegular: false },
...
function renderInvoiceTable() {
    ...
    if (logs.length === 0) {
        tbody.innerHTML = `<tr><td colspan="11" style="padding:48px;text-align:center;color:#94a3b8;font-size:13px;font-style:italic;">No records found.</td></tr>`;
        tfoot.innerHTML = '';
        return;
    }
    ...
    tbody.innerHTML = logs.map((l, i) => {
        ...
        totSales += r.totalSales; totVat += r.lessVat; totNet += r.netOfVat; totDisc += r.discount; totDue += r.totalDue;
        const rowBg = i % 2 === 0 ? '#fff' : '#f8fafc';
        return `<tr style="background:${rowBg};border-bottom:1px solid #f1f5f9;">
            <td style="padding:13px 16px;color:#94a3b8;">${i + 1}</td>
            <td style="padding:13px 16px;font-weight:700;color:#0f172a;">${l.name || '—'}</td>
            <td style="padding:13px 16px;color:#64748b;font-size:11px;">${l.address || '—'}</td>
            <td style="padding:13px 16px;font-family:monospace;color:#0f172a;">${l.or_number || '—'}</td>
            <td style="padding:13px 16px;color:#64748b;">${l.valid_id || '—'}</td>
            <td style="padding:13px 16px;text-align:right;font-family:monospace;font-weight:600;color:#0f172a;">${fmt(r.totalSales)}</td>
            <td style="padding:13px 16px;text-align:right;font-family:monospace;color:#64748b;">(${fmt(r.lessVat)})</td>
            <td style="padding:13px 16px;text-align:right;font-family:monospace;color:${r.discount > 0 ? '#10b981' : '#cbd5e1'};">${r.discount > 0 ? '(' + fmt(r.discount) + ')' : '—'}</td>
            <td style="padding:13px 16px;text-align:right;font-family:monospace;color:#0f172a;">${fmt(r.netOfVat)}</td>
            <td style="padding:13px 16px;text-align:right;font-family:monospace;font-weight:700;color:#10b981;">${fmt(r.totalDue)}</td>
        </tr>`;
    }).join('');

    tfoot.innerHTML = `<tr style="background:#f1f5f9;border-top:2px solid #e2e8f0;">
        <td colspan="5" style="padding:13px 16px;text-align:right;font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.08em;">
            Totals (${logs.length} record${logs.length !== 1 ? 's' : ''})
        </td>
        <td style="padding:13px 16px;text-align:right;font-family:monospace;font-weight:700;color:#0f172a;">${fmt(totSales)}</td>
        <td style="padding:13px 16px;text-align:right;font-family:monospace;font-weight:700;color:#64748b;">(${fmt(totVat)})</td>
        <td style="padding:13px 16px;text-align:right;font-family:monospace;font-weight:700;color:#10b981;">${totDisc > 0 ? '(' + fmt(totDisc) + ')' : '—'}</td>
        <td style="padding:13px 16px;text-align:right;font-family:monospace;font-weight:700;color:#0f172a;">${fmt(totNet)}</td>
        <td style="padding:13px 16px;text-align:right;font-family:monospace;font-weight:800;color:#10b981;font-size:14px;">${fmt(totDue)}</td>
    </tr>`;
}
...
function exportInvoiceCSV() {
    function doExport() {
        ...
        ws['!cols'] = [
            { wch: 5 }, { wch: 28 }, { wch: 30 }, { wch: 12 }, { wch: 12 },
        ...
        XLSX.writeFile(wb, `momocart-invoice-${dateStr}.xlsx`);
    }
