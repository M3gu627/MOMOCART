        ...
        <div id="ep-form-grid" style="flex:1;display:grid;grid-template-columns:1fr 1fr;min-height:0;border-top:1px solid #e2e8f0;">
              ...
              <div style="background:#fff;border-radius:20px;padding:24px;box-shadow:0 1px 4px rgba(0,0,0,0.06);display:flex;flex-direction:column;gap:14px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                  ...
                  <div>
                    <label style="display:block;font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:5px;">Employee Name</label>
                    <input id="ep-employee-name" type="text" value="${esc(log.employee_name||'')}" class="ep-input" placeholder="Staff on duty" style="width:100%;padding:12px 14px;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:12px;font-size:13px;box-sizing:border-box;outline:none;">
                  </div>
                </div>
...
async function submitEditPanel() {
    ...
    const data = {
        ...
        employee_name:    document.getElementById('ep-employee-name')?.value || '',
    ...
    try {
        ...
        if (result.success) {
            // Optimistic patch — update allLogs in memory so the table re-renders instantly
            const idx = allLogs.findIndex(l => parseInt(l.id, 10) === id);
            if (idx !== -1) {
                allLogs[idx] = {
                    ...allLogs[idx],
                    name:             data.name,
                    ...
                    amount_gcash:     data.amount_gcash,
                    additional_cash:  data.additional_cash,
                    additional_gcash: data.additional_gcash,
                    total:            data.total,
            ...
            closeEditPanel();
            renderCurrentTab(); // instant re-render from patched allLogs
            loadLogs();         // background sync with server
        } else {
